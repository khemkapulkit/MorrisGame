...
public class MiniMaxGameImproved {
	...
	{
		//char [] board = new char[] {'X','X', 'W', 'X', 'B', 'X', 'X', 'B', 'B', 'X', 'B', 'X', 'W', 'X', 'B', 'W', 'X', 'W', 'X', 'B', 'X', 'W', 'X'};
		...
		Output out= MiniMaxMidgameEndgame(Integer.parseInt(args[2]), true, board);
	...
	public static int statEstMidgameEndgame(char[] board)
	{
		...
		int posmills = possibleMillsWhite(board);
		...
		if (b <= 2) 
			...
			{return(-10000);}
				...
				if (numBlackMoves==0) 
					...
					{return ( (1000*(w + posmills - b)) - numBlackMoves);}

	}
	
	public static int possibleMillsWhite(char[] board)
	{
		int counter = 0;
		for (int i = 0; i< 23; i++)
		{
	...
	public static Output MiniMaxMidgameEndgame(int depth, boolean isWhite, char[] board)
	{
		...
		next.est = (isWhite) ? Integer.MIN_VALUE : Integer.MAX_VALUE;
		for (char[] b : nextMoves)
		{
			if (isWhite)
			{
				prev = MiniMaxMidgameEndgame(depth - 1, false, b);
				next.posEval += prev.posEval;
				next.posEval++;
				if (prev.est >= next.est)
				{
					next.est = prev.est;
					next.b = b;
				}
			...
			{
				prev = MiniMaxMidgameEndgame(depth - 1, true, b);
				next.posEval += prev.posEval;
				next.posEval++;
				if (prev.est <= next.est)
				{
